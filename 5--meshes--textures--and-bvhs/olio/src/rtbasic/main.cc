...
bool ParseArguments(int argc, char **argv, std::string *input_scene_name,
                    std::string *output_name, std::string *samples_per_pixel) {
...
main(int argc, char **argv)
{
  ...
  uint num_samples;
  if (!ParseArguments(argc, argv, &input_scene_name, &output_name, &samples_per_pixel))
    return -1;

  num_samples = (uint) stoi(samples_per_pixel);
  cout << " Output Name2: " << output_name << "\n" << endl;
  cout << " Samples2: " << num_samples << "\n" << endl;
  ...
  //BVHNode::Ptr BVH_n = BVHNode::Create();
  BVHNode bvh;
