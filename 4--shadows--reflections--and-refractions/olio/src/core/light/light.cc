...
PointLight::Illuminate(const HitRecord &hit_record, const Vec3r &view_vec, Surface::Ptr scene) const
{
  ...
  Ray ray(hit_record.GetPoint(), position_ - hit_record.GetPoint());
  HitRecord hit_record_temp;
  if(scene->Hit(ray, kEpsilon, 1, hit_record_temp))
    return black;
  /*for(auto Tsurf = scene.begin(); L != scene.end(); L++)
  {
    if((*Tsurf)->Hit(ray, kEpsilon, hit_record.GetRayT() - kEpsilon, hit_record))
      return black;
  }*/
